...
| dens_factors       |  *cf*   | False       | cluster density/scale factors<be>multiplies both step size and radius for cluster<br> assigns all clusters a density factor $1$ if False<br> assign random dens_factors *cf*$_{i}$ between $0.5$ and $2$ if True<br>can also be set per cluster in a list                                                                                                                  |
| momentum           | $\omega$     | $0.5$         | cluster momentum factors<br>assigns the value to all clusters as stickiness factor assigns<br>sets random stickiness factors between $0$ and $1$ if None<br>can also be set per cluster in a list                                                                                                                |
...
| distributions | *dist*     | $None$           | distributions for random generation of data points for each core<br>None or "uniform" uses uniform distribution<br>"gaussian" uses a multivariate Gaussian<br>"studentt" uses multivariate Student's t distribution with degrees of freedom 2<br>if a number si given, uses multivariate Student's t distribution with degrees of freedom corresponding to the given number<br>can also be set per cluster in a list |
...
| step |  | False | overwrite value for the step size $r_{shift}$<br>if set to False the step size ($r_{shift}$) is fixed to $radius\cdot \frac{dim}{2}$, which corresponds to both settings provided by Junhao Gan and Yufei Tao |
