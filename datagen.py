...
class densityDataGen:
    def __init__(self, dim=2, clunum=2, clu_ratios=None, core_num=10, min_ratio=0, ratio_noise=0, domain_size=1, radius=1, shift=1,
                 ratio_con = 0, connections=0, seed=0, dens_factors=False, stickiness=0, con_stickiness=0.9,  min_dist=1.1, con_min_dist = 0.9,
                 shift_spread=0, max_retry=5, verbose= False, safety=True, con_dens_factors = False, con_radius = 2, con_shift = 2, 
                 branch = 0, star = 0, square = False):
        ...
        if verbose:
            for i in range(len(connection_starts)):
                print("Connection from " + str(connection_starts[i]) + " to " + str(connection_stops[i]) + " with factor " + str(self.con_dens_factors[i]) + " and stickiness " + str(self.con_stickiness[i]))
        ...
        for i in range(len(connection_starts)):
            ...
            while(True):
                ...
                if noretry:
                    ...
                    if len(self.cores[conid]) == 0:
                        if retry >= self.max_retry:
                            ...
                            if(self.verbose):
                                print("Could not generate connection from " + str(connection_starts[i]) + " to " + str(connection_stops[i]) + " as they were too close")
                            break
                        else:
                            retry += 1
                    else:
                ...
                if retry >= self.max_retry:
                    self.cores.pop(conid, None)
                    if(self.verbose):
                        print("Could not generate connection from " + str(connection_starts[i]) + " to " + str(connection_stops[i]))
                    break
                else:
                    retry += 1
    ...
    def generate_cluster(self, cluid):
        ...
        while(True):
            ...
            if noretry:
                break
            elif (retry >= self.max_retry): #expand domain size, at some point there is enough space for new cluster
                self.domain_size = self.domain_size + self.r_sphere
                retry = 0
                if(self.verbose):
                    print("domain size increased")
            else:
                if(self.verbose):
                    print("restart")
                retry += 1
    ...
    def make_connection(self, startid, stopid, conid):
        ...
        min_dist = (self.con_dens_factors[conind] * self.c_sphere + self.dens_factors[stopid] * self.r_sphere) * self.con_min_dist
        retry_some = False
        tries_some = -1
        while(dist > min_dist):
            #print(dist)
        ...
        if len(connectors) > 0:
            ...
            pos = target + (self.con_dens_factors[conind] * self.c_sphere + self.dens_factors[startid] * self.r_sphere) * shift_dir * self.con_min_dist
            ...
            pos = target + (self.con_dens_factors[conind] * self.c_sphere + self.dens_factors[stopid] * self.r_sphere) * shift_dir * self.con_min_dist
    ...
    def paint(self, dim1, dim2, data = None, show_radius=True, show_core=True, cores = None):
        ...
        if data is not None:
            plt.scatter(data[:,dim1], data[:,dim2], color=color[data[:,len(data[0])-1].astype('int32')+1], alpha=1)
            ...
            if len(cores[cluid]) > 0:
                ...
                if show_radius:
                    for core in cores[cluid]:
                        #print(core[0])
                        if cluid >= 0:
                            plt.gca().add_patch(mpatches.Circle((core[dim1], core[dim2]), radius=self.r_sphere*self.dens_factors[cluid], color=color[cluid+1], alpha=0.2))
                        else:
                            plt.gca().add_patch(mpatches.Circle((core[dim1], core[dim2]), radius=self.c_sphere*self.con_dens_factors[(-1*cluid)-2], color=color[cluid+1], alpha=0.2))
            ...
            if command[i] == "(":
                ...
                if command[i+1] == "c":
                    ...
                    for conkey in self.connections.keys():
                        if(self.connections[conkey] == str(cid) + "-" +  str(eid) or self.connections[conkey] == str(eid) + "-" +  str(cid)):
                            cid = conkey
